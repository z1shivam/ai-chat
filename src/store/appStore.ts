...
import { MessageService, ConversationService, type DBMessage, type DBConversation } from '@/lib/database';
    ...
    (set, get) => ({
      ...
      updateProvider: (id: string, updatedProvider: ProviderConfig) => {
        set((state) => ({
          ...
          selectedProvider: state.selectedProvider?.id === id ? updatedProvider : state.selectedProvider,
        }));
      },
...
export const useCurrentConversation = () => useAppStore((state) => state.getCurrentConversation());
